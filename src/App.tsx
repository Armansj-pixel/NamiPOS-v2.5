...
const QRIS_IMG_SRC = "/qris.png"; // letakkan di /public/qris.png
...
type Shift = {
  id: string; outlet: string; openBy: string;
  openAt: Timestamp; closeAt?: Timestamp | null;
  openCash?: number; isOpen: boolean;
};

type PayMethod = "cash" | "ewallet" | "qris";
...
.logo{display:block;margin:0 auto 6px;height:40px}
</style></head><body>
<div class="wrap">
...
<script>window.print();</script>
  ...
  function clearSelection(){ setSelectedSaleIds(new Set()); }

  async function deleteOneSale(id: string) {
    ...
    try {
      ...
      setSelectedSaleIds(prev => { const n = new Set(prev); n.delete(id); return n; });
    } catch (e:any) {
  ...
  async function deleteSelectedSales() {
    ...
    if (!confirm(`Hapus ${selectedSaleIds.size} transaksi terpilih? Tindakan tidak bisa dibatalkan.`)) return;

    setDeleting(true);
    try {
      const batch = writeBatch(db);
      selectedSaleIds.forEach(id => batch.delete(doc(db, "sales", id)));
      await batch.commit();
      setHistoryRows(prev => prev.filter(s => !selectedSaleIds.has(s.id!)));
      clearSelection();
    } catch (e:any) {
      alert("Gagal menghapus sebagian/semua: " + (e?.message || e));
    } finally {
      setDeleting(false);
    }
  }
  ...
  /* =========================================
     OWNER: PRODUCTS & INVENTORY (CRUD ringkas)
  ========================================= */
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-4">
                ...
                {filteredProducts.map(p=>(
                  <button key={p.id} onClick={()=>addToCart(p)} className="bg-white rounded-2xl border p-3 text-left hover:shadow">
                    <div className="h-20 rounded-xl bg-gradient-to-br from-emerald-50 to-emerald-100 mb-2 overflow-hidden">
                      {p.imgUrl ? <img src={p.imgUrl} alt={p.name} className="h-20 w-full object-cover"/> : null}
                    </div>
            ...
            <div className="md:col-span-5">
              <div className="bg-white rounded-2xl border p-3">
                ...
                <div className="grid grid-cols-1 gap-2 mb-2">
                  <select className="border rounded-lg px-3 py-2" value={payMethod} onChange={e=>setPayMethod(e.target.value as PayMethod)}>
          ...
          <section className="bg-white rounded-2xl border p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                {isOwner && (
                  <button
                    className="px-3 py-2 rounded-lg border bg-rose-50 text-rose-700 disabled:opacity-50"
                    onClick={deleteSelectedSales}
                    disabled={deleting || selectedSaleIds.size===0}
                    title={selectedSaleIds.size ? `${selectedSaleIds.size} dipilih` : "Pilih transaksi dulu"}
                  >
                    {deleting ? "Menghapus..." : `Hapus Terpilih (${selectedSaleIds.size})`}
                  </button>
                )}
        ...
        {tab==="products" && isOwner && (
          <section className="bg-white rounded-2xl border p-3">
            ...
            <div className="overflow-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-left border-b"><th>Nama</th><th>Kategori</th><th>Harga</th><th>Gambar (URL)</th><th className="text-right">Aksi</th></tr></thead>
                <tbody>
                  {products.map(p=>(
                    <tr key={p.id} className="border-b">
                      <td className="py-2">
                        <input className="border rounded px-2 py-1 w-48" defaultValue={p.name} onBlur={(e)=>upsertProduct({ id:p.id, name:e.currentTarget.value, price:p.price, category:p.category, active:p.active, imgUrl:p.imgUrl })}/>
                      </td>
                      <td>
                        <input className="border rounded px-2 py-1 w-36" defaultValue={p.category||""} onBlur={(e)=>upsertProduct({ id:p.id, category:e.currentTarget.value, name:p.name, price:p.price, active:p.active, imgUrl:p.imgUrl })}/>
                      </td>
                      <td>
                        <input type="number" className="border rounded px-2 py-1 w-28" defaultValue={p.price} onBlur={(e)=>upsertProduct({ id:p.id, price:Number(e.currentTarget.value)||0, name:p.name, category:p.category, active:p.active, imgUrl:p.imgUrl })}/>
                      </td>
                      <td>
                        <input className="border rounded px-2 py-1 w-60" defaultValue={p.imgUrl||""} onBlur={(e)=>upsertProduct({ id:p.id, imgUrl:e.currentTarget.value, name:p.name, category:p.category, price:p.price, active:p.active })}/>
                      </td>
                      <td className="text-right">
                        <button className="px-2 py-1 border rounded mr-2" onClick={()=>upsertProduct({ id:p.id, active: !(p.active===false), name:p.name, price:p.price, category:p.category, imgUrl:p.imgUrl })}>
                ...
                <tbody>
                  {ingredients.map(i=>(
                    <tr key={i.id} className={`border-b ${i.min && i.stock<=i.min ? "bg-amber-50" : ""}`}>
                      <td className="py-2">
                        <input className="border rounded px-2 py-1 w-48" defaultValue={i.name} onBlur={(e)=>upsertIngredient({ id:i.id, name:e.currentTarget.value, unit:i.unit, stock:i.stock, min:i.min })}/>
                      </td>
                      <td>
                        <input className="border rounded px-2 py-1 w-24" defaultValue={i.unit} onBlur={(e)=>upsertIngredient({ id:i.id, unit:e.currentTarget.value, name:i.name, stock:i.stock, min:i.min })}/>
                      </td>
                      <td>
                        <input type="number" className="border rounded px-2 py-1 w-24" defaultValue={i.stock} onBlur={(e)=>upsertIngredient({ id:i.id, stock:Number(e.currentTarget.value)||0, name:i.name, unit:i.unit, min:i.min })}/>
                      </td>
                      <td>
                        <input type="number" className="border rounded px-2 py-1 w-24" defaultValue={i.min||0} onBlur={(e)=>upsertIngredient({ id:i.id, min:Number(e.currentTarget.value)||0, name:i.name, unit:i.unit, stock:i.stock })}/>
                      </td>
                      <td className="text-right">
